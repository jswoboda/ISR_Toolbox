function [Ne,Xi,Yi,Zi,utime,T1,T2] = interpAMISRtable(varargin)
%
%interpRISRnocal  Linear interpolation of non-calibrated AMISR data onto a
%cartesian grid from madrigal table data
%
%   SYNTAX:
%           interpAMISRtable
%           interpAMISRtable(StartTime,EndTime,file_name)
%
%   INPUT:
...
Range = All_Data.range;
N_e_log = All_Data.nel;
N_e = 10.^(N_e_log);
utime = All_Data.ut1_unix;
az = All_Data.azm * pi/180;  
el = All_Data.elm * pi/180; 
...
el1 = repmat(el,length(Range),1); 
az1 = repmat(az,length(Range),1); 

% Grab just the points within the desired altitude
alt_idx = find( Range>StartAlt & Range<EndAlt );
el2 = el1(alt_idx);
az2 = az1(alt_idx);
